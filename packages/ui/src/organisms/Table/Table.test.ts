...
test("renders accessible tabular data", () => {
  const markup = renderToStaticMarkup(createElement(Table, {
    ...
    columns: [{ key: "name", label: "Name" }, { key: "count", label: "Projects", align: "end" }],
...
test("renders a single accessible empty row", () => {
  const markup = renderToStaticMarkup(createElement(Table, { columns: [{ key: "name", label: "Name" }], rows: [], emptyMessage: "Nothing here" }));
